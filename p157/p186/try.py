...
pCaller = 0
pCalled = 0
journal = {} # caller: {all called}
for i in range(1000000):
    journal[i] = set()

for n in range(1, lim+1):
    journal[S[2*n-1]].add(S[2*n]) # append called to the caller
    #ppl[S[2*n]] += 1
    #ppl[S[2*n-1]] += 1
    if S[2*n-1] == S[2*n]:
...
nbF = 0 # nb of friend; they might be counted twice or more
for caller in journal:
    for called in journal[caller]:
        if called in journal: # from ~10*6 should be ok
            if caller in journal[called]:
                nbF += 1
print("Nb friend:{}".format(nbF))
